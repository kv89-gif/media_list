...
def newsapi_to_rows(articles: list[dict]) -> list[dict]:
...
def detect_columns(df: pd.DataFrame):
    ...
    for c in df.columns:
        ...
        # ---- domain authority / tier column ----
        if da_tier_col is None and ("domain authority" in lc or lc in {"da", "domain rating", "dr", "authority", "tier"} or "authority tier" in lc):
            da_tier_col = c
...
def contacts_to_rows(df: pd.DataFrame, niches: list[str], enrich_urls: bool) -> list[dict]:
    ...
    da_tier_col = cols.get("da_tier_col")
    notes_col = cols.get("notes_col")

    rows = []

    for _, r in df.iterrows():
        if not row_relevant_to_niches(r, niches, industry_col):
            continue

        pub = _safe_strip(r.get(pub_col)) if pub_col else ""
        email = _safe_strip(r.get(email_col)) if email_col else ""
        da_tier = _safe_strip(r.get(da_tier_col)) if da_tier_col else ""
...
def combined_unique_journalists(rows: list[dict]) -> pd.DataFrame:
    ...
    for r in rows:
        ...
        if r.get("Domain Authority Tier"):
            if _safe_strip(r.get("Domain Authority Tier")):
                b["DA Tiers"].add(_safe_strip(r.get("Domain Authority Tier")))
        if r.get("Notes"):
        ...
        out_rows.append(
            {
                ...
                "Domain Authority Tier": ", ".join(sorted(b["DA Tiers"])) if b["DA Tiers"] else "",
...
uploaded_contacts = st.sidebar.file_uploader(
    "Upload your CSV/XLSX (may contain writer names and/or article URLs)",
    type=["csv", "xlsx"],
)
...
if run_btn:
    ...
    if uploaded_contacts is not None:
        ...
        csv_rows = contacts_to_rows(contacts_df, niches=niches, enrich_urls=enrich_urls)
    ...
    if api_key and query:
        try:
            ...
            news_rows = newsapi_to_rows(articles)
